...
export class CreatorsComponent {  
  ...
  async connectWallet() {
    ...
    this.window.ethereum.request({ method: 'eth_requestAccounts' }).then((accounts:any) => {
      ...
      axios.post('http://localhost:3000/handle', {'address':address}).then(res => {        
  ...
  sendSigninForm(val: any) {
    ...
    axios.post('http://localhost:3000/addartist', val.value).then(res => {
      
      axios.post('http://localhost:3000/handle', {'address':this.address}).then(res => {        
        if (res.data.registered) {
          this.isAddressRegistered = true
          this.isAddressProvided = false
        } else {
